...
unsafe trait IJawsApi: IDispatch {
  ...
  fn SayString(&self, StringToSpeak: BSTR, bFlush: VARIANT_BOOL, vbSuccess: *mut VARIANT_BOOL) -> HRESULT;
...
impl SpeechSynthesizer for Jaws {
  ...
  fn data(&self) -> SpeechSynthesizerData {
    SpeechSynthesizerData { name: "JAWS".to_owned(), supports_to_audio_data: false, supports_to_audio_output: true, supports_speech_parameters: false }
  }
  fn list_voices(&self) -> std::result::Result<Vec<Voice>, SpeechError> {
    Ok(vec!(Voice { synthesizer: self.data(), display_name: "JAWS".to_owned(), name: "jaws".to_owned(), languages: vec!(), priority: 0 }))
  }
...
impl SpeechSynthesizerToAudioOutput for Jaws {
  fn speak(&self, _voice: &str, _language: &str, _rate: Option<u8>, _volume: Option<u8>, _pitch: Option<u8>, text: &str, interrupt: bool) -> std::result::Result<(), SpeechError> {
    let mut result: VARIANT_BOOL = Default::default();
    unsafe { self.jaws_api.SayString(text.into(), interrupt.into(), &mut result).ok()? };
    if result.into() {
      Ok(())
    }
    else {
      Err(SpeechError { message: "JAWS failed to speak".to_owned() })
    }
  }
