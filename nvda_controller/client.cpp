...
error_status_t nvdaController_initialize() {
	...
	RPC_STATUS status = RpcBindingFromStringBindingW(rpcWstr, &nvdaControllerBindingHandle);
	if (RPC_S_OK != status) {
		return status;
	}
	status = RpcBindingFromStringBindingW(rpcWstr, &nvdaController2BindingHandle);
	if (RPC_S_OK != status) {
		return status;
	}
	return RPC_S_OK;
}
error_status_t nvdaController_close() {
